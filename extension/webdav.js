/**
 * CipherVault Extension WebDAV & GitHub Gist Sync Client
 */
export class SyncClient {
  ...
  /**
   * Automatically search for existing ciphervault.json Gist
   */
  ...
  static async pushGist(config, encryptedString) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) throw new Error('GitHub API 访问权限不足 (请确保勾选 gist 权限)');
